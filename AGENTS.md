...
* Enhancer candidate analysis should use `active_enhancer_candidates` only. Do not use `poised_enhancer_candidates` for current analysis tables, case-study evidence counts, or publication-facing summaries unless the user explicitly asks for a separate exploratory comparison.
* Enhancer candidate tracks should be OFC-relevant only. The selected `cell` labels are `MESENCHYMAL` and `NEURALCREST` for `active_enhancer_candidates`. The `poised_enhancer_candidates` table is retained in the database for reference/backups, but it is not part of the current analysis convention.
...
* Case-study files live in `case_studies/` (committed to Git) so they travel with the repo when pushed/pulled. This includes per-gene `*.json` files plus `SUMMARY.csv`.
...
* The active curated table is `assets/table1_41.csv`, converted from `assets/table1_41.xlsx`. The workbook has 41 data rows plus four trailing footnote rows; the CSV excludes the footnote rows and preserves header footnote markers as Unicode superscripts. `pages/table.py`, Case Study expression lookups, and `case_studies/SUMMARY.csv` should all use this same 41-row table. Do not reintroduce old 43-gene or `table1_Shin_Tim_41` sources unless the user explicitly asks for an archival/internal comparison.
* A curated gene summary is stored in `case_studies/SUMMARY.csv`. `utils.case_studies.load_case_study_summary()` reads it and returns `{file, columns, rows}` or `None`. When present, the Case Study page renders it as the first sub-tab (`Summary Table`); the second sub-tab (`Case Studies`) contains the case-study dropdown, warnings, and selected case-study content.
* `scripts/convert_case_study_docx.py` converts `Case_Study_*.docx` sources into case-study JSON. It classifies paragraphs by label (`Function`, `IGV`, `Literature`, `Pathway`, `Supporting Data`), synthesizes `{text, links, suffix}` IGV (`/population?gene=<GENE>`) and Pathway (`/pathway?genes=...`) links, puts citation-like paragraphs in a "References" section and other unlabeled text under "EMT"/"Notes", and appends a "Supporting Data" evidence table computed via read-only DB queries (exon/enhancer/promoter overlap at freq <= 0.01) plus a gene-expression finding from `assets/table1_41.csv`. Run with `PYTHONPATH=. python scripts/convert_case_study_docx.py --input-dir <dir-with-docx>`; it writes `<gene>.json` into the committed `case_studies/` folder by default. The optional `--remote` flag uploads the generated JSONs + `SUMMARY.*` to OneDrive as a manual backup only (not pulled back). The resulting JSON for a gene named `TET3` overwrites the local `case_studies/TET3.json`.
...
* The Pathway page (`pages/pathway.py`) loads its edges/nodes from a local cache managed by `utils/pathway_networks.py`, with display checkboxes for Baseline, Extended, and Case Study 6 networks plus a Refresh button that runs `rclone sync`.
...
* `list_versions()` always includes bundled fallback networks and lets cache-discovered versions replace bundled entries with the same ID. Bundled fallbacks are `pathway/edges_e.csv`/`nodes_e.csv` (Baseline), `pathway/edges_v1.csv`/`nodes_v1.csv` (Extended), and `pathway/edges_v3.csv`/`nodes_v3.csv` (Case Study 6).
