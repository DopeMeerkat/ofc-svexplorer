"""
Random Forest Feature Importance — replicates visualization_uploader logic.
"""
...
def main() -> None:
    ...
    print(f"[random_forest] Loaded {len(df)} rows", flush=True)

    feature_df, target, target_col, features, excluded, model_df = prepare_feature_data(df, args.target_column)
    if excluded:
        print(f"[random_forest] Excluded columns: {excluded}", flush=True)
    print(f"[random_forest] Using {len(features)} features, target = {target_col}", flush=True)
    ...
    print(f"[random_forest] Model trained ({args.n_estimators} trees)", flush=True)
    ...
    print(f"[random_forest] Wrote feature_importance.csv", flush=True)

    model_df.to_csv(os.path.join(args.output_dir, "results.csv"), index=False)
    print(f"[random_forest] Wrote results.csv", flush=True)
    ...
    if HAS_PLOTLY and not importance_df.empty:
        ...
        fig = px.bar(
            ...
            title=f"Random Forest Feature Importance (target: {target_col})",
        ...
        print(f"[random_forest] Wrote plot.json", flush=True)
    ...
    print(f"[random_forest] Done in {elapsed}s", flush=True)
